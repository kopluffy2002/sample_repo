...
//   database: process.env.DB_NAME || "schedumingle",
// });
...
function connectWithRetry() {
  ...
  db.connect((err) => {
    if (err) {
      ...
      // start your express app / socket.io here
    }
  });
}
      ...
      if (err) {
        ...
        wss.clients.forEach((client) => {
          if (client.readyState === WebSocket.OPEN) {
            client.send(
              JSON.stringify({ sender, receiver, message, files, timestamp }),
            );
          }
        });
    ...
    (err, result) => {
      if (err) {
        ...
        wss.clients.forEach((client) => {
          if (client.readyState === WebSocket.OPEN) {
            client.send(
              JSON.stringify({ sender, group, message, files, timestamp }),
            );
          }
        });
...
app.post("/notifications/meeting", (req, res) => {
  ...
  wss.clients.forEach((client) => {
    if (
      ...
      receivers.includes(client.username)
    ) {
      client.send(JSON.stringify(notification));
    }
  });
